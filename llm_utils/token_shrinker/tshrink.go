...
// TShrinkTable holds columnar, CSV-friendly data (strings repeat; no pooling).
type TShrinkTable struct {
...
// TShrinkDocument is the final LOEF-like payload: multiple tables keyed by name.
type TShrinkDocument struct {
	Root   string                   `json:"root"`
	Tables map[string]*TShrinkTable `json:"tables"`
}

func BuildTShrinkDocuments(rootName string, payload []interface{}) (*TShrinkDocument, error) {
	return BuildTShrinkDocument(rootName, map[string]interface{}{rootName: payload})
}

// BuildTShrinkDocument takes a JSON payload (already unmarshaled) and flattens it into tables.
// Nested objects become separate tables; parents reference children via "<field>_id" (1-based).
func BuildTShrinkDocument(rootName string, payload map[string]interface{}) (*TShrinkDocument, error) {
	...
	doc := &TShrinkDocument{
		Root:   rootName,
		Tables: make(map[string]*TShrinkTable),
	}

	var encodeRow func(table string, row map[string]interface{}) (int, error)

	getTable := func(name string) *TShrinkTable {
	...
	addField := func(tbl *TShrinkTable, field string) {
	...
	encodeRow = func(table string, row map[string]interface{}) (int, error) {
		...
		if len(scalarKeys) > 1 {
			for i, k := range scalarKeys {
				if k == "name" && i != 0 {
					// move "name" to front
					scalarKeys = append([]string{"name"}, append(scalarKeys[:i], scalarKeys[i+1:]...)...)
					break
				}
			}
		}
...
// RenderTShrinkDocument renders the TShrinkDocument into a human-friendly CSV-like text (root first, then other tables).
func RenderTShrinkDocument(doc *TShrinkDocument) string {
	var b bytes.Buffer
...
func writeTable(b *bytes.Buffer, tbl *TShrinkTable) {
	if tbl == nil {
		return
	}
