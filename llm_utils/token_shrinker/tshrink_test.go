...
func Test_BuildTShrinkDocument_SamplePeople(t *testing.T) {
	raw := `{
		"people": [
	...
	doc, err := BuildTShrinkDocument("people", payload)
	if err != nil {
		t.Fatalf("BuildCSVDocument: %v", err)
	}
	out := RenderTShrinkDocument(doc)

	expected := `name,age,addresses_id
robin,20,[1]
kaleb,15,[1]
marco,25,[2]
---addresses
...
func Test_BuildTShrinkDocument_FourLevelNesting(t *testing.T) {
	raw := `{
		"root": [
	...
	doc, err := BuildTShrinkDocument("root", payload)
	if err != nil {
		t.Fatalf("BuildTShrinkDocument: %v", err)
	}
	out := RenderTShrinkDocument(doc)
...
func Test_BuildTShrinkDocument_FourLevelEmbeddedObjects(t *testing.T) {
	raw := `{
		"root": [
...
func Test_BuildTShrinkDocument_TenPeople_SharedAddressesCountries(t *testing.T) {
	raw := `{
		"people": [
